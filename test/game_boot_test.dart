...
//
// The tactical AI (see ai_strategy_test.dart, enemy_commander_test.dart) is
// a live, genuinely active opponent from the very first frame — including the
// zero-`dt` priming update `testWithGame` itself runs before any test body
// gets to touch the game. That means even tests that never call `_advance`
// can observe an enemy send that already happened during boot. Every test
// here is about the core loop (combat, capture, outcome), not the AI, so
// every game in this file is built with the AI strategy replaced by a no-op
// double that never acts, keeping `enemyBase` exactly as each test leaves it.

class _NoOpStrategy implements AIStrategy {
  @override
  List<AIAction> decideActions(AIDecisionContext context) => [];
}

TowerConquestGame _newGame() => TowerConquestGame(
    enemyCommander: EnemyCommander(strategy: _NoOpStrategy()));
...
void main() {
  group('Boot', () {
    testWithGame<TowerConquestGame>(
        'mounts both nodes and the route between them', _newGame, (game) async {
    ...
    testWithGame<TowerConquestGame>('the route joins the two nodes', _newGame,
        (game) async {
      expect(
    ...
    testWithGame<TowerConquestGame>(
        'nodes start with units and generate more', _newGame, (game) async {
  ...
  group('Sending a unit', () {
    testWithGame<TowerConquestGame>(
        'spends a unit and lights up the route', _newGame, (game) async {
      final before = game.playerBase.unitsInside;
    ...
    testWithGame<TowerConquestGame>(
        'the unit travels toward its target', _newGame, (game) async {
      final unit = game.sendUnit(from: game.playerBase, to: game.enemyBase)!;
    ...
    testWithGame<TowerConquestGame>(
        'the route goes idle once the unit arrives', _newGame, (game) async {
    ...
    testWithGame<TowerConquestGame>('an empty node cannot send', _newGame,
        (game) async {
  ...
  group('Arriving at a node', () {
    testWithGame<TowerConquestGame>('a friendly unit reinforces it', _newGame,
        (game) async {
    ...
    testWithGame<TowerConquestGame>(
        'a hostile unit wears the node down', _newGame, (game) async {
      game.enemyBase.generationRate = 0;
      final before = game.enemyBase.unitsInside;
    ...
    testWithGame<TowerConquestGame>(
        'a unit killed en route never lands a hit', _newGame, (game) async {
      game.enemyBase.generationRate = 0;
      final before = game.enemyBase.unitsInside;
  ...
  group('Path combat', () {
    ...
    testWithGame<TowerConquestGame>(
        'friendly units pass through each other', _newGame, (game) async {
      game.enemyBase.generationRate = 0;
  ...
  group('Match outcome', () {
    testWithGame<TowerConquestGame>(
        'capturing the last enemy node wins', _newGame, (game) async {
    ...
    testWithGame<TowerConquestGame>(
        'losing the last player node loses', _newGame, (game) async {
      game.playerBase.takeUnits(game.playerBase.unitsInside);
      game.playerBase.generationRate = 0;
    ...
    testWithGame<TowerConquestGame>('the match starts out running', _newGame,
        (game) async {
    ...
    testWithGame<TowerConquestGame>(
        'no more sends once the match is over', _newGame, (game) async {
      game.enemyBase.takeUnits(game.enemyBase.unitsInside);
      game.enemyBase.generationRate = 0;
      game.sendUnit(from: game.playerBase, to: game.enemyBase);
      await game.ready();
    ...
    testWithGame<TowerConquestGame>(
        'restart puts the match back to the start', _newGame, (game) async {
      game.enemyBase.takeUnits(game.enemyBase.unitsInside);
      game.enemyBase.generationRate = 0;
      game.sendUnit(from: game.playerBase, to: game.enemyBase);
      await game.ready();
